...
def draw_lines(img, lines):
    ...
    cv2.line(img, (int(next_frame[0]), int(next_frame[1])), (int(next_frame[2]), int(next_frame[3])), red, 10)
    cv2.line(img, (int(next_frame[4]), int(next_frame[5])), (int(next_frame[6]), int(next_frame[7])), red, 10)
...
def process_image(image):
    ...
    if image is None or len(image) == 0:
        # 이미지가 비어 있거나 유효하지 않은 경우에 대한 예외 처리
        print("Error: Empty or invalid image.")
        return
    image = cv2.resize(image, (1280, 720))
    image = image.astype(np.uint8)  # 8비트 부호 없는 정수로 변환
...
def visualize(image):
    if image is None or image.size == 0:
        print("Error: Empty or invalid image in visualize.")
        return
...
first_frame = 1
start = time.time()
image_name =  "test_videos/bot1.mp4"
