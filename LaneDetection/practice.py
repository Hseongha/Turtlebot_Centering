...
def process_image(image):
    ...
    img_hsv = cv2.cvtColor(resized_image, cv2.COLOR_RGB2HSV) # 더 넓은 폭의 노란색 범위를 얻기위해 HSV를 이용한다.
...
def visualize(result,l_x1, r_x2 ):
    ...
    font = cv2.FONT_HERSHEY_SIMPLEX
    lane_width_meters = 3.7  # 예시로 사용한 차선의 폭
    lane_width_pixels = r_x2 - l_x1 # 여기에 코드에서 계산한 차선의 픽셀 폭을 넣어야 합니다.
    xm_per_pix = lane_width_meters / lane_width_pixels

    left_or_right = 'Left' if l_center[0] < whalf else 'Right' # whalf : 영역의 가운데
    offset_text = 'Vehicle is {:.2f}m {} from the center'.format(np.abs(l_center[0] - whalf) * xm_per_pix,left_or_right)
    cv2.putText(result, offset_text, (50, 100), font, 1, (255, 255, 255), 2)
    ...
    if lane_center[0] < whalf-gap:
        ...
        cv2.putText(result, 'Turn Leftt', (190, hei), font, 1, red, font_size)
...
def Lane_Detection(frame):
    processing = process_image(frame)
    region = Region(processing, l_x1, r_x2)
    return region
...
# first_frame = 1
#
# image_name = "test_videos/drive.mp4"
# cap = cv2.VideoCapture(image_name)
# clip1 = save_video('out_videos/result_asd.mp4') # result 영상 저장
#
# while (cap.isOpened()):
#     _, frame = cap.read()
#     if frame is None :
